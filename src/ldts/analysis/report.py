...
def build_report(input_csv: Path | list[Path], output_dir: Path, categories: list[str] | None = None) -> Path:
    ...
    city_table = table("縣市申請數量", {x["city"]: x["case_count"] for x in city_summary})
...
<h2>合作網絡與 Portfolio Explorer</h2><p>下圖顯示登記資料中的醫療機構—認證實驗室關係。選擇任一方可查看合作對象與檢測項目；這不是檢體量或營收網絡。</p><div id="network" class="chart"></div><label for="entity">選擇醫療機構或實驗室：</label><select id="entity"></select><div id="details"></div>
<h2>登記縣市簡圖</h2><p>地圖使用縣市代表點；機構點為同縣市內的示意分散位置，不是實際地址或經緯度。</p><div id="map"></div>
{city_table}{table('醫療機構', summary['institution_counts'])}{table('認證實驗室', summary['laboratory_counts'])}{table('檢測類別', summary['categories'])}{table('Panel size', summary['panel_size_counts'])}
{price_table}
...
function drawNetwork(){{const base=filteredRows(), n=topSelect.value==='ALL'?999999:Number(topSelect.value), ic=counts(base,'institution'), topInstitutions=new Set(Object.keys(ic).sort((a,b)=>ic[b]-ic[a]).slice(0,n)), rows=topSelect.value==='ALL'?base:base.filter(r=>topInstitutions.has(r.institution)), edges={{}};rows.forEach(r=>{{if(r.institution&&r.laboratory){{const k=r.institution+'|||'+r.laboratory;edges[k]=(edges[k]||0)+1;}}}});const pairs=Object.entries(edges).map(([k,count])=>{{const [institution,laboratory]=k.split('|||');return {{institution,laboratory,count}};}});const ins=[...new Set(pairs.map(e=>e.institution))],labs=[...new Set(pairs.map(e=>e.laboratory))],nodes=[...ins,...labs],idx=new Map(nodes.map((x,i)=>[x,i]));Plotly.react('network',[{{type:'sankey',orientation:'h',node:{{label:nodes,color:nodes.map((x,i)=>i<ins.length?'#168aad':'#f08c46')}},link:{{source:pairs.map(e=>idx.get(e.institution)),target:pairs.map(e=>idx.get(e.laboratory)),value:pairs.map(e=>e.count),label:pairs.map(e=>e.count+' 案'),color:pairs.map(()=> '#b8c4ce')}}}}],{{title:'Top 醫療機構的全部合作網絡',font:{{size:12}}}},{{responsive:true,displaylogo:false}});}}
const select=document.getElementById('entity');
const rawTables=[...document.querySelectorAll('table')]; rawTables.slice(0,3).forEach(t=>t.style.display='none'); [...document.querySelectorAll('h2')].filter(h=>['醫療機構','認證實驗室','檢測類別'].includes(h.textContent.trim())).forEach(h=>h.style.display='none'); const entitySection=document.createElement('section'); entitySection.innerHTML='<h2>縣市機構與實驗室清單</h2><label for="entity_city">縣市：</label><select id="entity_city"><option value="ALL">全部縣市</option></select><div id="entity_lists"><table id="institution_list"><tr><th>醫療機構</th><th>案件數</th></tr></table><table id="lab_list"><tr><th>認證實驗室</th><th>案件數</th></tr></table></div><div id="top10_institutions" class="chart"></div><div id="top10_laboratories" class="chart"></div>'; document.body.insertBefore(entitySection,rawTables[0]); const entityCity=document.getElementById('entity_city'); [...new Set(data.network.rows.map(r=>r.city).filter(Boolean))].sort().forEach(c=>{{const o=document.createElement('option');o.value=c;o.textContent=c;entityCity.appendChild(o);}});
function renderEntityLists(){{const c=entityCity.value, rows=data.network.rows.filter(r=>c==='ALL'||r.city===c), ic=counts(rows,'institution'),lc=counts(rows,'laboratory');function renderTable(id,obj,label){{const t=document.getElementById(id);t.innerHTML='<tr><th>'+label+'</th><th>案件數</th></tr>'+Object.entries(obj).sort((a,b)=>b[1]-a[1]).map(([k,v])=>'<tr><td>'+k+'</td><td>'+v+'</td></tr>').join('');}}renderTable('institution_list',ic,'醫療機構');renderTable('lab_list',lc,'認證實驗室');const top=(obj)=>Object.entries(obj).sort((a,b)=>b[1]-a[1]).slice(0,10);function pie(id,title,obj){{const x=top(obj);Plotly.react(id,[{{type:'pie',labels:x.map(a=>a[0]),values:x.map(a=>a[1]),textinfo:'percent',hovertemplate:'%{{label}}<br>案件數：%{{value}}<extra></extra>'}}],{{title,margin:{{t:60,b:35,l:80,r:80}},height:400}},{{responsive:true,displaylogo:false}});}}pie('top10_institutions','Top 10 醫療機構（全臺）',counts(data.network.rows,'institution'));pie('top10_laboratories','Top 10 認證實驗室（全臺）',counts(data.network.rows,'laboratory'));}} entityCity.addEventListener('change',renderEntityLists);renderEntityLists();
function updateEntities(){{select.innerHTML='';const rows=filteredRows();const ins=[...new Set(rows.map(r=>r.institution).filter(Boolean))].sort();const labs=[...new Set(rows.map(r=>r.laboratory).filter(Boolean))].sort();[...ins.map(x=>['醫療機構',x]),...labs.map(x=>['認證實驗室',x])].forEach(([kind,name])=>{{const o=document.createElement('option');o.value=kind+'|'+name;o.textContent=kind+'：'+name;select.appendChild(o);}});if(select.options.length){{select.selectedIndex=0;showDetails();}}else{{document.getElementById('details').textContent='此縣市目前沒有可顯示的登記關係。';}}}}
function showDetails(){{if(!select.value)return;const [kind,name]=select.value.split('|'); const matching=filteredRows().filter(r=>kind==='醫療機構'?r.institution===name:r.laboratory===name); const partners=[...new Set(matching.map(r=>kind==='醫療機構'?r.laboratory:r.institution))]; const tests=[...new Set(matching.map(r=>r.test).filter(Boolean))]; document.getElementById('details').innerHTML='<p><b>'+name+'</b>｜案件 '+matching.length+'｜合作'+(kind==='醫療機構'?'實驗室':'醫療機構')+' '+partners.length+' 家</p><p>合作對象：'+partners.join('、')+'</p><p>檢測項目：'+tests.join('、')+'</p>';}} select.addEventListener('change',()=>{{showDetails();drawNetwork();}});topSelect.addEventListener('change',()=>{{drawCharts();drawNetwork();}});
citySelect.addEventListener('change',()=>{{drawCharts();updateEntities();drawNetwork();}});drawCharts();updateEntities();drawNetwork();
const networkNode=document.getElementById('network'), entityNode=document.getElementById('entity'), detailsNode=document.getElementById('details'), chartNode=document.getElementById('institutions'); const networkHeading=[...document.querySelectorAll('h2')].find(x=>x.textContent.includes('Portfolio Explorer')); const networkIntro=networkHeading?.nextElementSibling; if(networkHeading)chartNode.parentNode.insertBefore(networkHeading,chartNode); if(networkIntro)chartNode.parentNode.insertBefore(networkIntro,chartNode); const entityLabel=document.createElement('label');entityLabel.textContent='選擇醫療機構或認證實驗室：';entityLabel.htmlFor='entity';chartNode.parentNode.insertBefore(entityLabel,chartNode); chartNode.parentNode.insertBefore(entityNode,chartNode); chartNode.parentNode.insertBefore(networkNode,chartNode); chartNode.parentNode.insertBefore(detailsNode,chartNode);
